...
You can display time in a different timezone by adding a timezone offset to time-related
placeholders.
...
special placeholder called `{cycle|XX|YY}`, which alternates between `XX` and `YY` at the configured
cycle interval (default 1000ms, configurable in General settings).
...
Each cell cycles through the colors but offset by one position, creating a traffic light
animation. Adjust "{cycle} interval" in General settings to control animation speed.
